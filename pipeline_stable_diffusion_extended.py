...
class StableDiffusionPipelineExtended(StableDiffusionPipeline):
    ...
    """
        ...
        if do_classifier_free_guidance:
            ...
            # do not do CFG, in this case `unconditional_prompt_embeds` is just a dummy
            guided_prompt_embeds = prompt_embeds
            unconditional_prompt_embeds = torch.zeros_like(guided_prompt_embeds)
        ...
        assert len(image) == latents_list[0].shape[0] == unconditional_prompt_embeds.shape[0] == guided_prompt_embeds.shape[0] == log_prob_list[0].shape[0]

        return (
            image,
            latents_list,
            unconditional_prompt_embeds.cpu(),
            guided_prompt_embeds.cpu(),
            log_prob_list,
            kl_path,
        )
    ...
    """
        ...
        if unet_copy is not None:
            if do_classifier_free_guidance:
                ...
                del old_noise_pred_uncond, old_noise_pred_text
                # now we get the predicted noise
            log_prob_init = self.scheduler.step_forward_logprob(
                old_noise_pred, unet_times, latents, next_latents, **extra_step_kwargs
            )
        else:
            log_prob_init = torch.zeros(noise_pred.shape[0], device=noise_pred.device, dtype=noise_pred.dtype)

        log_prob = self.scheduler.step_forward_logprob(
            noise_pred, unet_times, latents, next_latents, **extra_step_kwargs
        )

        return log_prob, log_prob_init
